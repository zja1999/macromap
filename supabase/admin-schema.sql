...
create policy "data_requests_admin_read" on public.data_requests
  for select using ((auth.jwt() ->> 'email') in ('zja1999@gmail.com'));
...
create policy "data_requests_admin_update" on public.data_requests
  for update using ((auth.jwt() ->> 'email') in ('zja1999@gmail.com'))
  with check ((auth.jwt() ->> 'email') in ('zja1999@gmail.com'));
...
create policy "data_requests_admin_delete" on public.data_requests
  for delete using ((auth.jwt() ->> 'email') in ('zja1999@gmail.com'));
